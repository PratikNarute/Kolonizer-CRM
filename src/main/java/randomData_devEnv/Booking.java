...
//		String amount=OfferAmount_AreaWise.getAttribute("value"); a.assertEquals(amount, "25");
		...
		calenderButton.click();
		Calender.currentCalenderYear(driver);
		Calender.currentCalenderMonth(driver);
		Calender.currentCalenderDate(driver);
		
	//	A.sendKeys(Keys.PAGE_DOWN).build().perform();
		String RemainingInstallment_2=remainingInstallment_2.getAttribute("value"); a.assertEquals(RemainingInstallment_2, "575");
		...
		A.sendKeys(Keys.PAGE_DOWN).build().perform();
		Thread.sleep(1000); cheackBox_AutomationTermCondition.click();

		a.assertAll();		
	}

	public void submitButton_LeadClosure() throws InterruptedException
	{
		j.executeScript("arguments[0].scrollIntoView()", submitButton_LeadClosure);
		Thread.sleep(2000); submitButton_LeadClosure.click();
	}
	...
	{	
			...
			Thread.sleep(1000);automationProjectTwo_propertyNo_1BHK_2.click();
	...
	public void primaryContactInformation_bookingPropertyThroughLead() throws InterruptedException
	{
		primaryContact_leadRefrenceName.sendKeys(this.name_bookedLead);
		Thread.sleep(1000); primaryContact_leadRefrenceName_AtomationBookedLead.click();
	...
	public void veiwPrebookingsButton()
	{
