...
public class ProjectSetup_TermAndCondition {
	...
	public void addTag() throws InterruptedException, EncryptedDocumentException, IOException
	{
		...
		addTermAndCondition_Button.click();	
		for (int termConditons=1; termConditons<=3; termConditons++)
		{
			System.out.println("Start to create tag: "+Excel.importDataFromExcel("Term and Conditions", termConditons, 0));
			Thread.sleep(2000);
			tagIcon.click();
			Thread.sleep(1000); searchOrCreateTag.sendKeys(Excel.importDataFromExcel("Term and Conditions", termConditons, 0)+" "+utility.UtilityClass.getRandomString(1));
	...
	public void addTermAndConditions() throws InterruptedException, EncryptedDocumentException, IOException
	{
		for (int termConditons=1; termConditons<=3; termConditons++)
		{
			System.out.println("Start to create term and condition: "+Excel.importDataFromExcel("Term and Conditions", termConditons, 1));
			Thread.sleep(2000);
			projectSetupIcon.click();
			Wait.explicitWait(termAndCondition_Icon, 10, driver);
			termAndCondition_Icon.click();
		    ...
		    name.sendKeys(Excel.importDataFromExcel("Term and Conditions", termConditons, 1)+" "+utility.UtilityClass.getRandomString(2));
		    Wait.explicitWait(description, 10, driver);
		    description.sendKeys(Excel.importDataFromExcel("Term and Conditions", termConditons, 2)+" "+UtilityClass.getRandomString(2));
		    Wait.explicitWait(tagIcon, 10, driver);
		    tagIcon.click();
		    Wait.explicitWait(searchOrCreateTag, 10, driver);
			searchOrCreateTag.sendKeys(Excel.importDataFromExcel("Term and Conditions", termConditons, 0));
			listOfElements = driver.findElements(By.xpath("//p[contains(text(), '"+Excel.importDataFromExcel("Term and Conditions", termConditons, 0)+"')] / parent :: label"));
