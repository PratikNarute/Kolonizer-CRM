	...
	//====================================================================================================================================================
		...
		public void CreateOffters() throws InterruptedException, ParseException, EncryptedDocumentException, IOException
		{
			...
			for (int offer=1; offer<=4; offer++)
			{
				...
				offerName.sendKeys(Excel.importDataFromExcel("Offers", offer, 0)+" "+utility.UtilityClass.getRandomString(1));
				Wait.explicitWait(offerType_Dropdown, 10, driver);
				offerType_Dropdown.click();
				Thread.sleep(1000); 
				
				String offerType=Excel.importDataFromExcel("Offers", offer, 1);
				System.out.println("Start to create offer: "+offerType);
				if (offer==1)
				{
					driver.findElement(By.xpath("//mat-option//span[contains(text(), '"+Excel.importDataFromExcel("Offers", offer, 1)+"')]")).click();
					discountPerSqFt.sendKeys(Excel.importDataFromExcel("Offers", offer, 2));
				}
				else if (offer==2)
				{
					WebElement list_ProductName=driver.findElement(By.xpath("//mat-option//span[contains(text(), '"+Excel.importDataFromExcel("Offers", offer, 1)+"')]"));
					...
					product_Description.sendKeys(Excel.importDataFromExcel("Offers", offer, 2));
				...
				else if (offer==3)
				{
					...
					DirectDiscountInRS.sendKeys(Excel.importDataFromExcel("Offers", offer, 2));
				}
				else
				{
				...
				Thread.sleep(1000);
				driver.findElement(By.xpath("//mat-option//span[contains(text(), '"+Excel.importDataFromExcel("Offers", offer, 6)+"')]")).click();
				
				Thread.sleep(1000); propetyPriceMoreThan.sendKeys(Excel.importDataFromExcel("Offers", offer, 7));
				
				WebElement applicableForSpecificProject_RadionButton=driver.findElement(By.xpath("//input[@type='radio' and @value='"+Excel.importDataFromExcel("Offers", offer, 8)+"']"));
				applicableForSpecificProject_RadionButton.click();
				
				if (Excel.importDataFromExcel("Offers", offer, 8).contains("Yes"))
				{
					Wait.explicitWait(projectDropdown, 10, driver);
					projectDropdown.click();
					Thread.sleep(1000);
					driver.findElement(By.xpath("//mat-option//span[contains(text(), '"+Excel.importDataFromExcel("Offers", offer, 9)+"')]")).click();
